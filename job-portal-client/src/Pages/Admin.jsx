...
const Admin = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(
      `https://job-portal-back-4yxs.onrender.com/all-jobs?sort=-postingDate`
    ) // sorts by postingDate
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const filteredItems = jobs.filter((job) => {
      ...
      const companyNameMatch =
        job.companyName.toLowerCase().indexOf(searchText.toLowerCase()) !== -1;
  ...
  const handleDelete = (id) => {
    fetch(`https://job-portal-back-4yxs.onrender.com/job/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const toggleJobApproval = (id) => {
    ...
    fetch(`https://job-portal-back-4yxs.onrender.com/job/${id}`, {
      ...
      body: JSON.stringify({
        isApproved: updatedJobs.find((job) => job._id === id).isApproved,
      }),
    })
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="bg-[#FAFAFA] flex flex-wrap gap-8 lg:px-24 px-4 py-12">
        ...
        <section className="flex-1 py-1 bg-blueGray-50">
          <div className="w-full mb-12 xl:mb-0 px-4 mx-auto">
            {" "}
            {/*// "w-full xl:w-8/12 mb-12 xl:mb-0 px-4 mx-auto mt-5"*/}
